...
ActivationString = tp.Literal["swish", "gelu", "relu"]


def _get_activation_fn(activation: str) -> tp.Callable[[Tensor], Tensor]:
...
def retention_parallel(
    ...
    decay_gammas: tp.Optional[Tensor] = None,
...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(
        ...
        activation: tp.Union[ActivationString, tp.Callable[[Tensor], Tensor]] = "swish",
...
class RetNetDecoderLayer(nn.Module):
    ...
    def forward_recurrent(
        self, x: Tensor, seq_idx: int, prev_state: tp.Optional[Tensor] = None
    ) -> tp.Tuple[Tensor, Tensor]:
        def _retention_block(x: Tensor) -> tp.Tuple[Tensor, Tensor]:
    ...
    def forward_chunkwise(
        self, x: Tensor, start_idx: int, prev_state: tp.Optional[Tensor] = None
    ) -> tp.Tuple[Tensor, Tensor]:
        def _retention_block(x: Tensor) -> tp.Tuple[Tensor, Tensor]:
...
class RetNetDecoder(nn.Module):
    ...
    def forward_recurrent(
        self, x: Tensor, seq_idx: int, prev_states: tp.Sequence[tp.Optional[Tensor]] = ()
    ...
    def forward_chunkwise(
        ...
        x: Tensor,
        start_idx: int,
        prev_states: tp.Sequence[tp.Optional[Tensor]] = (),
    ) -> tp.Tuple[Tensor, tp.List[Tensor]]:
        if not prev_states:
            prev_states = [None] * self.num_layers
        elif len(prev_states) != len(self.layers):
