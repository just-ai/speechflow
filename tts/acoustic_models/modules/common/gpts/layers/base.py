...
class Transformer(nn.Module):
    """It implements seq2seq Transformer TTS for debug(No StopPredictor and
    SpeakerEmbeding) Neural Speech Synthesis with Transformer Network
    ...
    def __init__(
        ...
        self.text_embedding = TokenEmbedding(d_model, self._num_text_tokens)  # W_x
        ...
        if add_prenet:
            ...
            self.decoder_prenet = nn.Sequential(
                nn.Linear(self._num_mel_bins, 256),
                nn.ReLU(),
                nn.Dropout(0.5),
                nn.Linear(256, 256),
                nn.ReLU(),
            ...
            if scaling_xformers:
                self.decoder_prenet = ScaledLinear(self._num_mel_bins, d_model)
            else:
                self.decoder_prenet = nn.Linear(self._num_mel_bins, d_model)
        ...
        if scaling_xformers:
            self.encoder = TransformerEncoder(
                TransformerEncoderLayer(
                    ...
                    linear2_self_attention_cls=partial(ScaledLinear, initial_scale=0.01),
            ...
            self.predict_layer = ScaledLinear(d_model, self._num_mel_bins)
            ...
            self.predict_layer = nn.Linear(d_model, self._num_mel_bins)
            self.stop_layer = nn.Linear(d_model, 1)
    ...
    def inference(
        ...
        **kwargs,
        ...
        y = torch.zeros(
            [x.shape[0], 1, self._num_mel_bins], dtype=torch.float32, device=x.device
        )
        while True:
            y_emb = self.decoder_prenet(y)
            y_pos = self.decoder_position(y_emb)
