...
class Framewise(nn.Module):
    ...
    def __init__(
        ...
        if cnn_model == "adapt":
            ...
            assert (
                ms_n_mels == 48
            ), f"ms_n_mels is {ms_n_mels} and should be 48, use adaptive model or change ms_n_mels"
            assert (
                ms_seg_length == 15
            ), f"ms_seg_len is {ms_seg_length} should be 15, use adaptive model or change ms_seg_len"
            assert (kernel_size == 3) or (
                kernel_size == (3, 3)
            ), f"cnn_kernel_size is {kernel_size} should be 3, use adaptive model or change cnn_kernel_size"
...
class AdaptCNN(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = nn.Conv2d(
            self.conv1.out_channels, self.c_out_2, self.kernel_size, padding=self.cnn_pad
        )

        self.bn2 = nn.BatchNorm2d(self.conv2.out_channels)

        self.conv3 = nn.Conv2d(
            self.conv2.out_channels, self.c_out_3, self.kernel_size, padding=self.cnn_pad
        )

        self.bn3 = nn.BatchNorm2d(self.conv3.out_channels)

        self.conv4 = nn.Conv2d(
            self.conv3.out_channels, self.c_out_3, self.kernel_size, padding=self.cnn_pad
        )

        self.bn4 = nn.BatchNorm2d(self.conv4.out_channels)

        self.conv5 = nn.Conv2d(
            self.conv4.out_channels, self.c_out_3, self.kernel_size, padding=self.cnn_pad
        )
...
def predict_dim(model, ds, dev, num_workers=0):
    """
    predict_dim: predicts MOS and dimensions of the given dataset with given
    model. Used for NISQA_DIM model.
    """
    xb, yb, (idx, n_wins) = ds[0]
    xb = xb.unsqueeze(0).to(dev)
    n_wins = torch.IntTensor([int(n_wins)])

    with torch.no_grad():
        y_hat_list = model(xb, n_wins).cpu().numpy()[0]

    return {
        "mos_pred": float(y_hat_list[0]),
        "noi_pred": float(y_hat_list[1]),
        "dis_pred": float(y_hat_list[2]),
        "col_pred": float(y_hat_list[3]),
        "loud_pred": float(y_hat_list[4]),
    }
    ...
    def update_bias(self, y, y_hat):
        ...
        if self.apply_bias_loss:
            ...
            if not self.do_update:
                ...
                if self.do_print:
                    print(f"--> bias update: min_r {r:0.2f}, r_p {self.min_r:0.2f}")
                if r > self.min_r:
                    self.do_update = True
...
class earlyStopper:
    ...
    Training is stopped if neither RMSE or Pearson's correlation is improving after
    "patience" epochs.
...
def segment_specs(file_path, x, seg_length, seg_hop=1, max_length=None):
    """Segment a spectrogram into "seg_length" wide spectrogram segments. Instead of using
    only the frequency bin of the current time step, the neighboring bins are included as
    input to the CNN. For example for a seg_length of 7, the previous 3 and the follwing 3
    frequency bins are included.

    A spectrogram with input size [H x W] will be segmented to: [W-(seg_length-1) x C
    x H x seg_length], where W is the width of the original mel-spec (corresponding
    to the length of the speech signal), H is the height of the mel-spec
    (corresponding to the number of mel bands), C is the number of CNN input Channels
    (always one in our case).

    """
    if seg_length % 2 == 0:
        raise ValueError(f"seg_length must be odd! (seg_lenth={seg_length})")
    ...
    if max_length is not None:
        if max_length < n_wins:
            raise ValueError(
                f"n_wins {n_wins} > max_length {max_length} --- {file_path}. Increase max window length ms_max_segments!"
            )
